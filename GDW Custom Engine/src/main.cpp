...
int main() {
	...
	if (loadScene) {
		...
		Guid handMesh = ResourceManager::CreateMesh("hand2.obj");
		Guid boxTexture = ResourceManager::CreateTexture("textures/box-diffuse.png");
		Guid carpetTexture = ResourceManager::CreateTexture("textures/CarpetFloor.png");
		...
		MaterialInfo::Sptr carpetMaterial = std::make_shared<MaterialInfo>();
		carpetMaterial->Shader = scene->BaseShader;
		carpetMaterial->Texture = ResourceManager::GetTexture(carpetTexture);
		carpetMaterial->Shininess = 8.0f;
		scene->Materials[carpetMaterial->GetGUID()] = carpetMaterial;
		...
		scene->Camera->SetPosition(glm::vec3(12, 12, 6));
		scene->Camera->LookAt(glm::vec3(0.0f));
		scene->Camera->SetUp(glm::vec3(0.0f, 0.0f, -1.0f));
		...
		RenderObject hand = RenderObject();
		hand.Position = glm::vec3(0.0f, 0.0f, 1.0f);
		hand.Mesh = ResourceManager::GetMesh(handMesh);
		hand.Material = boxMaterial;
		hand.Rotation.z = 180.0f;
		hand.Rotation.x = 90.0f;
		hand.Scale = glm::vec3(0.1);
		hand.Name = "Hand";
	...
	RenderObject* hand = scene->FindObjectByName("Hand");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (isDebugWindowOpen) {
			...
			if (DrawSaveLoadImGui(scene, scenePath)) {
				...
				hand = scene->FindObjectByName("Hand");
			}
			ImGui::Separator();
		}
		...
		{
			if (hand->Position.x > -4)
			{
				hand->Position.x -= dt * 3;
			}
		}
		...
		{
			if (hand->Position.y > -4)
			{

				hand->Position.y -= dt * 3;
			}
		}
		...
		{
			if (hand->Position.z > 0)
			{
				hand->Position.z -= dt * 3;
			}
		}
		if (glfwGetKey(window, GLFW_KEY_LEFT_ALT) == GLFW_PRESS)
		{
			if (hand->Position.z < 4)
			{
				hand->Position.z += dt * 3;
			}
		}
